...
def create_full_prot(
    ...
    if chain_index is None:
        ...
        chain_index = chain_index.cpu().numpy().astype(int) - 1 # for zero indexing
        ...
        if prot_pos.ndim == 4:
            for t, pos37 in enumerate(prot_pos):
                ...
                prot = create_full_prot(
                    pos37, atom37_mask, aatype=aatype, chain_index=chain_index, b_factors=b_factors)
                pdb_prot = protein.to_pdb(prot, model=t + 1, add_end=False)
                f.write(pdb_prot)
        elif prot_pos.ndim == 3:
            atom37_mask = np.sum(np.abs(prot_pos), axis=-1) > 1e-7
            prot = create_full_prot(
                prot_pos, atom37_mask, aatype=aatype, chain_index=chain_index, b_factors=b_factors)
