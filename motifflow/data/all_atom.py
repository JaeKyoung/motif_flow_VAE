...
################ ATOM14 ################
########################################
# https://github.com/levinthal/Pallatom/blob/main/alphafold/model/all_atom.py

def _batched_gather(params, indices, axis=0, batch_dims=0):
    """Implements a NumPy equivalent of `tf.gather` with `axis` and `batch_dims`."""
    ...
    # If there are no batch dimensions, just use np.take directly
    if batch_dims == 0:
        # Prepare gather dim
        dims = list(range(len(params.shape)))
        dims.insert(0, dims.pop(axis))
        inverse_dims = list(range(len(dims)))
        inverse_dims.insert(axis, inverse_dims.pop(0))
        
        params = params.permute(dims)
        gathered = torch.gather(params, 0, indices)
        return gathered.permute(inverse_dims)
    ...
    assert param_shape[:batch_dims] == index_shape[:batch_dims], \
        "Batch dimensions must match between params and indices"
    
    # Reshape params and indices to combine batch dimensions
    combined_batch_size = torch.prod(torch.tensor(param_shape[:batch_dims])).item()
    params_reshaped = params.reshape((combined_batch_size,) + param_shape[batch_dims:])
    indices_reshaped = indices.reshape((combined_batch_size,) + index_shape[batch_dims:])
    
    # Gather for each batch element
    gathered = torch.stack([
        torch.gather(p, axis, i) 
        for p, i in zip(params_reshaped, indices_reshaped)
    ])
    
    # Reshape back to original batch dimensions
    output_shape = index_shape[:batch_dims] + gathered.shape[1:]
...
def atom14_to_atom37(
    ...
    atom37_data = _batched_gather(atom14_data,
