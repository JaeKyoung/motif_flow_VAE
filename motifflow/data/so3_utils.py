...
def geodesic_w(w: float, mat: torch.Tensor, base_mat: torch.Tensor, rot_vf=None) -> torch.Tensor:
    """
    Computes the geodesic interpolation for CFG.
    Specifically, R_w = Exp_{base_mat}(w * Log_{base_mat}(mat)).

    Args:
        w: weight for CFG scaling.
        mat: target points on manifold.
        base_mat: source point on manifold.
        rot_vf: optional pre-computed rotation vector field.

    Returns:
        Point along geodesic with CFG weight w applied.
    """
    if rot_vf is None:
        rot_vf = calc_rot_vf(base_mat, mat)
    mat_w = rotvec_to_rotmat(w * rot_vf)
    if base_mat.shape != mat_w.shape:
        raise ValueError(
            f'Incompatible shapes: base_mat={base_mat.shape}, mat_w={mat_w.shape}')
    return torch.einsum("...ij,...jk->...ik", base_mat, mat_w)
