...
from motifflow.data import residue_constants
import numpy as np
import collections
import string
import pickle
...
from motifflow.data import protein
import dataclasses
from Bio import PDB
import io
...
# Convert amino acid type to sequence
...
def batch_align_structures(pos_1, pos_2, mask=None):
    """
    Align two sets of structures in batch mode.
    Align pos_1 to pos_2
    """
...
def adjust_oxygen_pos(atom_37: torch.Tensor, pos_is_known = None) -> torch.Tensor:
    ...
    carbonyl_to_oxygen_term = (calpha_to_carbonyl_term + calpha_to_nitrogen_term) # (N, 3)
    ...
    next_res_gone = ~pos_is_known.bool() # (N,)
...
def chain_str_to_int(chain_str: str):
    """Convert chain string to integer representation."""
    chain_int = 0
    if len(chain_str) == 1:
...
def parse_chain_feats(chain_feats, scale_factor=1.):
    ...
    # chain_feats : aatype, atom_mask, bb_mask, atom_positions, bb_positions, modeled_idx, b_factors, chain_index
...
def parse_chain_feats_no_center(chain_feats, scale_factor=1.):
    # Parse and process chain features without centering
    # chain_feats : aatype, atom_mask, bb_mask, atom_positions, bb_positions, modeled_idx, b_factors, chain_index
    ca_idx = residue_constants.atom_order['CA']
    chain_feats['bb_mask'] = chain_feats['atom_mask'][:, ca_idx]
...
def parse_pdb_feats(
        ...
        pdb_path: str,
        scale_factor=1.,
        chain_id='A', # None으로 두면 알아서 찾음. Inference시에는 그렇게하는데, Train시에는?
        centering = True
    ):
    """
    ...
    Args:
        ...
        chain_id: Chain ID(s) to process.

    Returns:
        Dict with CHAIN_FEATS features extracted from PDB with specified preprocessing.
    """
    parser = PDB.PDBParser(QUIET=True)
    structure = parser.get_structure(pdb_name, pdb_path)
    struct_chains = {chain.id: chain for chain in structure.get_chains()}

    def _process_chain_id(x, centering=True):
        ...
        if centering:
            ...
            return parse_chain_feats_no_center(feat_dict, scale_factor=scale_factor)

    if isinstance(chain_id, str):
        return _process_chain_id(chain_id)
    elif isinstance(chain_id, list):
        return {x: _process_chain_id(x) for x in chain_id}
    elif chain_id is None and centering:
        return {x: _process_chain_id(x) for x in struct_chains}
    elif chain_id is None and not centering:
        return {x: _process_chain_id(x, centering=False) for x in struct_chains}
    else:
...
def process_chain(chain: Chain, chain_id: str) -> Protein:
    ...
    Args:
        ...
        chain_id: ID of the chain being processed.
...
def align_pos(
    ...
    T: np.ndarray, [3]
        Translation vector.
    """
    assert prb_pos.shape == ref_pos.shape, (prb_pos.shape, ref_pos.shape)
    assert prb_pos.ndim == 2, prb_pos.shape
    assert prb_pos.shape[-1] == 3, prb_pos.shape
